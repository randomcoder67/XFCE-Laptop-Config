...
local tethysStyle = {
    ...
    speedText = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(tethys.textColor, tethys.speedTextSize, tethys.osdSymbolFont),
    resolutionText = ("{\\blur0\\bord0\\1c&H%s\\3c&HFFFFFF\\fs(%d)}"):format(tethys.textColor, tethys.resolutionTextSize, tethys.osdSymbolFont),
...
layouts["tethys"] = function()
    ...
    local tcW = (state.tc_ms) and 170 or 110
    local tsW = 90
    local minW = (buttonW + padX)*5 + (tcW + padX)*4 + (tsW + padX)*2 + 100
    ...
    lo = add_layout("speed_text")
    lo.geometry = geo
    lo.style = tethysStyle.speedText
    if elements["speed_text"].visible then
        rightSectionWidth = rightSectionWidth + geo.w
    end
    ...
    -- Resolution Text
    resLen = string.len(elements["resolution_text"].content())
    geo = {
        x = rightX - rightSectionWidth,
        y = line1Y + buttonH/2,
        an = 6, -- x,y is right-center
    ...
    lo = add_layout("resolution_text")
    lo.geometry = geo
    lo.style = tethysStyle.resolutionText
    if elements["resolution_text"].visible then
        rightSectionWidth = rightSectionWidth + geo.w
    end
...
function osc_init()
    ...
    ne = new_element("speed_text", "button")
    --ne.visible = tethys.showSpeedButton
    ne.content = function()
        local curSpeed = state.speed
        return string.format("%.2fx", mp.get_property_osd("speed"))
    end
    
    ne = new_element("resolution_text", "button")
    --ne.visible = tethys.showSpeedButton
    ne.content = function()
        local curSpeed = state.speed
        return string.format("%sp%s", mp.get_property_osd("height"), mp.get_property_osd("container-fps"))
    end
