...
for video in "${videos[@]}"; do
	...
	playlistJSON=$(yt-dlp -J --cookies-from-browser firefox --flat-playlist --no-warnings "$video")
	...
	if [[ "$2" == "-l" ]]; then
		yt-dlp --external-downloader aria2c --external-downloader-args aria2c:"-x 16 -j 16 -s 16 -k 1M --max-overall-download-limit=3M" -o "%(title)s.%(ext)s" --embed-chapters --cookies-from-browser firefox "$video"
	else
		yt-dlp --external-downloader aria2c --external-downloader-args aria2c:"-x 16 -j 16 -s 16 -k 1M" -o "%(title)s.%(ext)s" --embed-chapters --cookies-from-browser firefox "$video"
	fi
